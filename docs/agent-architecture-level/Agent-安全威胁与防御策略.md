...
:::caution 黑名单永远不完整，用白名单
只允许已知安全的操作，而不是试图阻止所有危险操作。
:::
...
:::warning Agent 必须以最小权限运行
创建专用的低权限用户，只赋予必要的文件和网络访问权限。
:::
...
:::caution 日志永远不记录敏感信息
使用安全日志过滤器，自动遮蔽密码、Token、API Key 等字段。
:::
...
:::tip 用 LLM 做"意图分类"
在执行前先让 LLM 判断用户意图是否可疑，作为额外的安全层。
:::
